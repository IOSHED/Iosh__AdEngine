...
#[utoipa::path(
    ...
    path = "/client/bulk",
    tag = "Client",
    request_body = Vec<domain::schemas::UserProfileSchema>,
    responses(
        (status = 201, description = "Bulk client creation", body = Vec<domain::schemas::UserProfileSchema>),
...
pub async fn client_bulk_handler(
    register_data: actix_web::web::Json<Vec<domain::schemas::UserProfileSchema>>,
...
#[utoipa::path(
    ...
    tag = "Client", responses(
        (status = 200, description = "Get client by id", body = domain::schemas::UserProfileSchema),
        (status = 400, description = "Bad request", body = interface::actix::exception::ExceptionResponse),
